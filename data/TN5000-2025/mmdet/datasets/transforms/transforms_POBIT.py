...
class RandomFlip(MMCV_RandomFlip):
    ...
    def convert_bit_planes_to_image(self, b_bits, g_bits, r_bits, img_size):
        ...
        ###pil_img= Image.merge('RGB', (r_aug, g_aug, b_aug))
    ...
    def bit_plane_slice1(self, b_bits, g_bits, r_bits, bit_plane_list, x1, y1, h, w):
        if bit_plane_list is not None:
            tmp=random.uniform(0, 1)
            for bit_plane in bit_plane_list:
                ###logging.debug('Zeroizing {} bit_plane'.format(int(bit_plane)))
                # zeroize the bit plane in each RGB channel
                b_bits[x1:x1+w, y1:y1+h,int(bit_plane)] = 0
                g_bits[x1:x1+w, y1:y1+h,int(bit_plane)] = 0
                r_bits[x1:x1+w, y1:y1+h,int(bit_plane)] = 0
                '''
                b_bits[x1:x1+w, y1:y1+h,random.randint(2, 7)] = 0
                g_bits[x1:x1+w, y1:y1+h,random.randint(2, 7)] = 0
                r_bits[x1:x1+w, y1:y1+h,random.randint(2, 7)] = 0
    ...
    def _flip(self, results: dict) -> None:
        ...
        if random.uniform(0, 1) < 0.5:     
            img2=copy.deepcopy(results['img'])
            ###print(results['img'].shape)
            height, width, channels = results['img'].shape
            img_size = (height, width)
            bit_size = img_size + (8,)


            sal_mask = BMS_thresh(img2,0.05) # Threshold for saliency map
            ...
            for reg in sal_regions:
                ...
                if random.uniform(0, 1) < 0.5:
                    ...
                    self.bit_plane_slice1(b_bits, g_bits, r_bits, tmp2, x1, y1, h, w)
            ...
            b_bits, g_bits, r_bits = self.convert_image_to_bit_planes(img2, bit_size)
            self.bit_plane_slice(b_bits, g_bits, r_bits, tmp2)
            img_bit = self.convert_bit_planes_to_image(b_bits, g_bits, r_bits, img_size)
            results['img']=img_bit
            '''
